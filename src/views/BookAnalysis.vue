...
【分析维度】
1. 写作技法：叙事视角、描写手法、修辞运用等
2. 结构特点：章节布局、情节推进、节奏把控等
3. 人物塑造：角色设定、性格刻画、关系处理等
4. 语言风格：文字特色、对话设计、情感表达等
5. 创作亮点：新颖设定、独特视角、精彩片段等
...
【分析重点】
1. 章节结构：章节划分、开篇结尾、过渡衔接等
2. 情节布局：起承转合、伏笔照应、高潮设置等
3. 叙事节奏：快慢变化、张弛有度、悬念营造等
4. 线索设计：主线副线、明线暗线、多线并进等
...
【分析维度】
1. 人物设定：身份背景、外貌特征、能力设定等
2. 性格刻画：言行举止、心理活动、成长变化等
3. 关系处理：人物互动、情感纠葛、利益冲突等
4. 塑造手法：直接描写、侧面烘托、对比反衬等
...
【分析角度】
1. 文字风格：朴实华丽、幽默严肃、古典现代等
2. 修辞手法：比喻拟人、排比对比、象征暗示等
3. 句式特点：长短句搭配、特殊句式运用等
4. 语言特色：地域特色、时代特色、个人风格等
...
【分析重点】
1. 情节推进：铺垫发展、转折高潮、结局收尾等
2. 冲突设计：内部冲突、外部冲突、多层冲突等
3. 悬念技巧：设悬解悬、信息控制、读者期待等
4. 戏剧张力：情感冲突、利益对立、生死考验等
...
const loadAnalysisTemplates = () => {
  ...
  if (savedPrompts) {
    try {
      ...
      const promptsFromStorage = allPrompts
      ...
      // 如果从 localStorage 加载到模板，合并内置默认模板
      if (promptsFromStorage.length > 0) {
        analysisTemplates.value = [...defaultAnalysisTemplates, ...promptsFromStorage]
      } else {
        // 如果没有用户自定义模板，使用内置默认模板
        analysisTemplates.value = defaultAnalysisTemplates
      }
    } catch (error) {
...
onMounted(() => {
  ...
  // 加载模板后，默认选中第一个
  nextTick(() => {
    if (analysisTemplates.value.length > 0 && !selectedTemplate.value) {
      selectedTemplate.value = analysisTemplates.value[0].id
    }
  })
})
</script>
